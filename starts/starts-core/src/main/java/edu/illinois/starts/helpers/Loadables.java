...
public class Loadables implements StartsConstants {
    ...
    public List<String> getClasspath(boolean withJars) {
        ...
        if (surefireClasspath != null) {
            for (String path : surefireClasspath.getClassPath()) {
                if ((withJars || !path.endsWith(JAR_EXTENSION)) && new File(path).exists()) {
                    localPaths.add(path);
                }
            }
        }
    ...
    public Map<String, Set<String>> getDepMap(String pathToUse, List<String> classes)
        ...
        List<String> localPaths = getClasspath(useThirdParty);
